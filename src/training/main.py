...
def main():
    ...
    if args.online_learning  and not args.online_cache:
        OL = OnlineLearner('valid', args)
        print('Online Learner Init.')
    else:
        OL = None
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        num_steps_this_epoch = train(model, data, epoch, optimizer, scaler, scheduler, args, steps, OL)
        ...
        if args.val_data is not None and args.valid_epoch_interval is not None and ((epoch + 1) % args.valid_epoch_interval) == 0:
            ...
            evaluate(model, optimizer, scaler, data, epoch, args, steps, OL)
